...
export default function ArticleSlugClient({ slug }: ArticleSlugClientProps) {
  ...
  const handleUpdateComplete = (updatedArticle?: any) => {
    ...
    // Check if the slug has changed (title was updated)
    if (updatedArticle && updatedArticle.slug && updatedArticle.slug !== slug) {
      console.log('Slug changed from', slug, 'to', updatedArticle.slug, '- redirecting');
      ...
      {article && (
        ...
        <div className={articleStyles.pageContainer} style={{ position: 'relative', zIndex: 1 }}>
          ...
          <aside className={articleStyles.relatedTagsSection}>
            ...
            <Link href='/create-article' className={articleStyles.writeButton}>
              Write
            </Link>
          </aside>
        </div>
